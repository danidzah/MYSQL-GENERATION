...
CREATE TABLE IF NOT EXISTS `db_blog_pessoal`.`tb_usuarios` (
  ...
  `tb_postagem_id` BIGINT NOT NULL,
  `tb_temas_id` BIGINT NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_tb_usuarios_tb_postagem1_idx` (`tb_postagem_id` ASC) VISIBLE,
  INDEX `fk_tb_usuarios_tb_temas1_idx` (`tb_temas_id` ASC) VISIBLE,
  CONSTRAINT `fk_tb_usuarios_tb_postagem1`
    FOREIGN KEY (`tb_postagem_id`)
    REFERENCES `db_blog_pessoal`.`tb_postagem` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_tb_usuarios_tb_temas1`
